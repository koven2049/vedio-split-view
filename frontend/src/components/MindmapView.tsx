...
export default function MindmapView({ videoId, platform, videoVideoId }: MindmapViewProps) {
  ...
  const streamError =
    streamErrorRaw === MINDMAP_GENERIC_STREAM_ERROR ? t('mindmap.generationFailed') : streamErrorRaw
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-16 gap-2 items-center text-sm" style={{ color: 'var(--color-text-secondary)' }}>
  ...
  if (notGenerated && !hasChapters) {
    return (
      <div className="rounded-xl p-10 flex flex-col items-center text-center max-w-md mx-auto" style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)' }}>
        ...
        {generating && (
          <p className="text-sm mt-4 flex items-center gap-2" style={{ color: 'var(--color-text-secondary)' }}>
            <Loader2 size={14} className="animate-spin" />
            {progressLabel}
          </p>
        )}
        {streamErrorRaw && (
          <p className="text-sm mt-4 flex items-center gap-2" style={{ color: 'var(--color-danger)' }}>
            <AlertCircle size={14} />
            {streamError}
          </p>
        )}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        {stats && (
          <div className="text-xs font-medium tabular-nums" style={{ color: 'var(--color-text-secondary)' }}>
      ...
      {streamErrorRaw && (
        <div className="p-3 rounded-lg text-sm flex items-center gap-2" style={{ background: 'rgba(239,68,68,0.08)', color: 'var(--color-danger)' }}>
          <AlertCircle size={16} />
          {streamError}
        </div>
      )}
      ...
      {generating && hasChapters && (
        <div className="flex items-center gap-2 text-sm px-3 py-2 rounded-lg" style={{ background: 'var(--color-bg-tertiary)', color: 'var(--color-text-secondary)' }}>
            ...
            >
              ...
              {points.length > 0 && (
                <ul className="space-y-1.5 mb-4">
                  {points.map((kp, j) => (
                    <li key={j} className="text-sm leading-relaxed" style={{ color: 'var(--color-text)' }}>
      ...
      {regenConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <button type="button" className="absolute inset-0 bg-black/40 backdrop-blur-sm cursor-default" aria-label={t('common.dismiss')} onClick={() => setRegenConfirmOpen(false)} />
          <div
