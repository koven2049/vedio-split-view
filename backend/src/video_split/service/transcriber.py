...
def split_audio(audio_path: Path, *, chunk_seconds: int | None = None) -> list[Path]:
    ...
    previous chunk instead of becoming its own. A 5701s audio at 300s chunks
    would otherwise produce a 1-second final chunk, which is usually silence or
    outro music ��� and Fun-ASR rejects speechless audio with
    ``ASR_RESPONSE_HAVE_NO_WORDS``, failing the whole job.

    Uses ``-f segment`` with stream copy for 67× speed over decode-encode loops.
    """
    settings = get_settings()
    ...
    tail = total_duration - (num_chunks - 1) * chunk_dur
    if num_chunks > 1 and tail < MIN_TAIL_SECONDS:
        num_chunks -= 1
        logger.info(
            "[split] Merging %.1fs tail into the final chunk (%d chunks instead of %d)",
            tail, num_chunks, num_chunks + 1,
        )
...
async def transcribe_audio(
    ...
    # Chunk N starts at exactly N * chunk_duration in the original audio, so
    # offsets come from the split boundaries — not from where the previous
    # chunk's last sentence happened to end. Deriving the offset from content
    # drifts backwards on every chunk with trailing silence, and stalls
    # completely on a chunk that yields no speech at all.
    chunk_dur_seconds = float(settings.transcription.chunk_duration_seconds)
    ...
    for i, chunk_path in enumerate(chunks):
        ...
        time_offset = 0.0 if is_single else i * chunk_dur_seconds
    ...
    # Per-chunk silence is tolerated above, but a recording with no speech
    # anywhere means the audio is wrong (silent download, music-only track).
    # Fail here rather than handing an empty transcript to the LLM stage.
    if not all_entries:
        raise RuntimeError(
            f"转录结果为空：{total} 个音频块均未识别到语音。"
            "音频可能是静音、纯音乐，或下载不完整。"
        )
...
# DashScope reports "this audio has no recognizable speech" as a task FAILURE.
# For one chunk of a long recording that is a normal outcome (silence, outro
# music), so it degrades to an empty transcript instead of killing the job.
_NO_SPEECH_MARKERS = ("HAVE_NO_WORDS", "NO_VALID_AUDIO", "SILENT")


def _is_no_speech(code: str, message: str) -> bool:
    blob = f"{code} {message}".upper()
    return any(marker in blob for marker in _NO_SPEECH_MARKERS)
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(30.0)) as client:
        ...
        while True:
            ...
            if status == "FAILED":
                ...
                # "no words" means the audio genuinely carries no speech
                # (silence, outro music, applause). That is a valid outcome for
                # one chunk of a long recording — not a reason to discard the
                # other 19 chunks that transcribed fine.
                if _is_no_speech(code, msg):
                    logger.warning(
                        "[funasr] Task %s: no speech detected (%s) — treating as empty chunk",
                        task_id, code or msg,
                    )
                    return [], 0.0
