...
def _duration_from_next_data(html: str, video_id: str) -> int:
    """Exact episode duration in seconds from Next.js hydration data, else 0.

    JSON-LD only carries ``timeRequired`` rounded down to whole minutes
    (``PT95M`` for a 95:09 episode), which truncates up to 59s off the tail —
    the player then can't reach the end and the last segment stops short.
    ``__NEXT_DATA__`` carries the exact second count for the same episode.
    """
    m = re.search(
    ...
    try:
        ...
        logger.debug("[xiaoyuzhou] __NEXT_DATA__ is not valid JSON")
        return 0
    ...
    def walk(node: object) -> int:
        ...
        if isinstance(node, dict):
            ...
            if isinstance(raw, (int, float)) and raw > 0:
                seconds = int(raw)
                # Prefer the object describing *this* episode; a podcast page
                # embeds sibling episodes with their own durations.
                if node.get("eid") == video_id:
                    return seconds
                if not fallback and "eid" not in node:
                    fallback = seconds
...
async def extract_xiaoyuzhou_metadata(url: str) -> tuple[VideoMeta, str]:
    ...
    duration_seconds = _duration_from_next_data(page_html, video_id)
    if duration_seconds <= 0:
        duration_seconds = _iso8601_duration_to_seconds(str(ld.get("timeRequired", "") or ""))
        if duration_seconds > 0:
            logger.info(
                "[xiaoyuzhou] exact duration unavailable; using minute-rounded %ds for %s",
