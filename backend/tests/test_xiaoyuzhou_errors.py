...
def _next_data(episodes: list[dict]) -> str:
    """Minimal __NEXT_DATA__ blob shaped like 小宇宙's hydration payload."""
    return (
        '<script id="__NEXT_DATA__" type="application/json">'
        + json.dumps({"props": {"pageProps": {"episodes": episodes}}})
        + "</script>"
    )


@pytest.mark.asyncio
async def test_duration_prefers_exact_seconds_over_rounded_minutes(monkeypatch):
    """WHY: JSON-LD timeRequired truncates to whole minutes (PT95M for 95:09),
    so the player can never reach the real end and the final segment is cut
    short. __NEXT_DATA__ carries exact seconds — it must win."""
    from video_split.service import xiaoyuzhou

    eid = "6a7bf13836641f136d86f3cc"
    html = _build_html(
        title="Vol.229",
        audio="https://cdn.xiaoyuzhoufm.com/a.m4a",
        json_ld=_podcast_episode_ld(duration="PT95M"),
    ) + _next_data([{"eid": eid, "title": "Vol.229", "duration": 5709}])
    _patch_async_client(monkeypatch, html=html)

    meta, _ = await xiaoyuzhou.extract_xiaoyuzhou_metadata(
        f"https://www.xiaoyuzhoufm.com/episode/{eid}"
    )
    assert meta.duration_seconds == 5709


@pytest.mark.asyncio
async def test_duration_picks_matching_episode_not_sibling(monkeypatch):
    """WHY: a page embeds sibling episodes with their own durations; matching on
    eid prevents adopting a neighbour's length."""
    from video_split.service import xiaoyuzhou
...
@pytest.mark.asyncio
async def test_duration_falls_back_to_json_ld_without_next_data(monkeypatch):
    """WHY: rounded duration is still better than zero — a missing hydration
    blob must not regress to 'duration unknown'."""
    from video_split.service import xiaoyuzhou
    ...
    meta, _ = await xiaoyuzhou.extract_xiaoyuzhou_metadata(
        "https://www.xiaoyuzhoufm.com/episode/6a7bf13836641f136d86f3cc"
    )
    assert meta.duration_seconds == 95 * 60
