...
- 运行时不要给容器 `--dns`：自定义网络上它只会被 aardvark 收成唯一上游，公网解析单点失败即 `EAI_AGAIN`。`podman_dns` 只用于 `rebuild`。开机用 `./manage.sh install-launchd`（系统 LaunchDaemon，需 sudo 一次）；`scripts/launchd-start.sh` 必须常驻巡检，不能 `start` 完就退出。
